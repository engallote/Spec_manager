...
public class CertificationModify extends AppCompatActivity{
        ...
        if (v.getId() == R.id.cancel) {
            ...
            try{
                ...
                if(name2.length() > 0 && number2.length() > 0 && (year2.length() == 4) && (month2.length() == 2)  && (day2.length() == 2) && agency2.length() > 0)
                {
                    ...
                    sdb.execSQL("UPDATE cerDB SET name='"+name2+"', year='"+year2+"', month='"+month2+"', day='"+day2+"', number='"+number2+"', agency='"+agency2+"' WHERE id = " + ID + ";");
